...
$('document').ready(function(){
	...
	$('#balloons_flying').click(function(){
		...
		$(this).fadeOut('slow').delay(5000).promise().done(function(){
			// Sau khi bóng bay lên, nút "Ai đây dạ" sẽ xuất hiện
			$('#who').fadeIn('slow');
		});
	});
	
	$('#who').click(function(){
		// Khi nhấn nút "Ai đây dạ", ảnh Quynh sẽ xuất hiện
		$('.quynh').fadeIn('slow');
		$(this).fadeOut('slow');  // Ẩn nút "Ai đây dạ" sau khi đã nhấn
		$(this).delay(3000).promise().done(function(){
			// Sau khi ảnh Quynh hiện ra, nút "Happy Birthday" sẽ xuất hiện
			$('#cake_fadein').fadeIn('slow');
		});
	});
	
	$('#cake_fadein').click(function(){
		// Khi nhấn nút "Mụt chiếc bánh", chiếc bánh sẽ xuất hiện
		$('.cake').fadeIn('slow');
		$(this).fadeOut('slow').delay(3000).promise().done(function(){
			// Sau khi bánh xuất hiện, đốt nến
